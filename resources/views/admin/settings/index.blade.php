            ...
            <form action="{{ route('settings.store') }}" method="POST" class="space-y-8">
                ...
                <div x-data="bankHandler()">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <button type="button" @click="addBank" class="btn btn-sm btn-gradient-primary">+ Tambah</button>
                    </div>
                    <table class="table w-full text-sm">
                        ...
                        <tbody>
                            <template x-for="(bank,index) in banks" :key="index">
                                <tr>
                                    <td><input type="text" :name="'bank_accounts['+index+'][name]'" x-model="bank.name" class="input input-bordered w-full"></td>
                                    <td><input type="text" :name="'bank_accounts['+index+'][number]'" x-model="bank.number" class="input input-bordered w-full"></td>
                                    <td><button type="button" @click="removeBank(index)" class="btn btn-gradient-error btn-xs">Hapus</button></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
                ...
                <div x-data="ewalletHandler()">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <button type="button" @click="addEwallet" class="btn btn-sm btn-gradient-primary">+ Tambah</button>
                    </div>
                    <table class="table w-full text-sm">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <template x-for="(wallet,index) in wallets" :key="index">
                                <tr>
                                    <td><input type="text" :name="'ewallets['+index+'][provider]'" x-model="wallet.provider" class="input input-bordered w-full"></td>
                                    <td><input type="text" :name="'ewallets['+index+'][number]'" x-model="wallet.number" class="input input-bordered w-full"></td>
                                    <td><button type="button" @click="removeEwallet(index)" class="btn btn-gradient-error btn-xs">Hapus</button></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
...
function ewalletHandler() {
    return {
        ...
        addEwallet() {
            this.wallets.push({provider:'', number:''});
        },
        removeEwallet(index) {
