    ...
    <div class="flex items-center justify-center px-4 md:px-0">
        <div class="bg-white shadow-xl rounded-2xl p-6 md:p-8 max-w-3xl w-full border border-blue-100">
            ...
            <div class="grid md:grid-cols-2 gap-8 items-start">
                @foreach ($plans as $plan)
                    ...
                    <form method="POST" action="{{ route('upgrade.account.payment') }}"
                        ...
                        <input type="hidden" name="final_price" id="final_price_{{ $plan->id }}"
                            value="{{ $plan->price }}">

                        {{-- Label Populer --}}
                        @if ($plan->is_popular)
                            <div
                                class="absolute -top-4 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-4 py-1 rounded-full text-sm font-semibold">
                                Paling Populer
                            </div>
                        @endif
                        ...
                        <div class="mb-6">
                            ...
                            <p id="diskonInfo{{ $plan->id }}" class="text-green-600 text-sm font-medium mt-1 hidden">
                            </p>
                        </div>
                        ...
                        @unless ($isCurrent)
                            ...
                            @else
                                <button type="button"
                                    class="w-full py-3 rounded-lg bg-gray-400 text-white font-semibold cursor-not-allowed">
                                    Coming Soon
                                </button>
                            @endif
                        @endunless
            ...
            <div class="text-center mt-4">
                <a href="/" class="inline-block text-blue-600 hover:underline text-sm font-medium">Lanjutkan ke
                    Beranda</a>
            </div>
        </div>
    </div>
                    ...
                    if (data.valid) {
                        ...
                        @foreach ($plans as $plan)
                            const hargaAkhir{{ $plan->id }} = data.is_percent ?
                                hargaNormal{{ $plan->id }} - (hargaNormal{{ $plan->id }} * data.amount /
                                    100) :
                                hargaNormal{{ $plan->id }} - data.amount;

                            const hemat{{ $plan->id }} = hargaNormal{{ $plan->id }} -
                                hargaAkhir{{ $plan->id }};

                            harga{{ $plan->id }}.textContent =
                                `Rp ${hargaAkhir{{ $plan->id }}.toLocaleString('id-ID')}`;
                        ...
                        @foreach ($plans as $plan)
                            harga{{ $plan->id }}.textContent =
                                `Rp ${hargaNormal{{ $plan->id }}.toLocaleString('id-ID')}`;
